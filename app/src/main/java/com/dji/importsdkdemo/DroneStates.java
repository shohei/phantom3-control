...
public class DroneStates {
    ...
    DroneStates(final Aircraft drone) {
        drone.getFlightController().setStateCallback(flightControllerState -> {
            pitch = (float) flightControllerState.getAttitude().pitch;
            yaw =  (float) flightControllerState.getAttitude().yaw;
            roll =  (float) flightControllerState.getAttitude().roll;
